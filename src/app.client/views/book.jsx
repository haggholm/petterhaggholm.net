...
function getAuthorRow(book) {
  if (book.hasMultipleAuthors) {
    return [
      ...
      [m('ul',
        _map(book.authors, (a) => (
          m('li', [m(`a href=[/authors/${a.id}]`, [a.full_name])])
        ))
      )]
    ];
...
function getTranslationRows(book) {
  ...
  ].concat(_each(book.translations, (t) => [
    'Translation',
    m2('cite', [t.title]),
    'by',
    _map(t.translators, (tr) => tr.full_name).join(', ')
  ]));
}
